...
def csv_confirm_import(request, workspace_id):
    """Persist the mapped CSV as a background import job and show live progress.

    The heavy per-row work (Post/PlatformPost creation, scheduling, category &
    tag handling) now happens in ``apps.composer.csv_import`` under the
    ``process_tasks`` worker. Here we only snapshot the wizard's CSV into a
    self-contained ``CSVImportJob`` (file + column mapping + row count), enqueue
    it, and hand back the polling progress partial.
    """
    import csv
    import io
    ...
    from .tasks import process_csv_import_job_task
    ...
    # Rebuild a self-contained CSV (header row + data rows) so the job owns its
    # file and the worker no longer depends on the upload session.
    buffer = io.StringIO()
    ...
    job.file.save(f"import-{job.id}.csv", ContentFile(csv_bytes), save=True)

    # Enqueue background processing (runs under `manage.py process_tasks`).
    process_csv_import_job_task(str(job.id))

    # The job is now the source of truth — drop the wizard's session state.
...
def csv_import_status(request, workspace_id, job_id):
    """Return the live progress partial, or the result partial once finished.

    Polled by HTMX. While the job is pending/processing this re-renders the
    progress partial (which keeps polling); once the job is completed or failed
    it renders the completion partial, which omits the polling trigger so HTMX
    stops on its own.
    """
    workspace = _get_workspace(request, workspace_id)
    job = get_object_or_404(
        CSVImportJob.objects.for_workspace(workspace.id),
        id=job_id,
    )

    if job.status in (CSVImportJob.Status.COMPLETED, CSVImportJob.Status.FAILED):
        template = "composer/partials/csv_complete.html"
    else:
        template = "composer/partials/csv_progress.html"

    return render(request, template, {"workspace": workspace, "job": job})
